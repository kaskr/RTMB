...
The `fft` function, available in R, is essentially a method to perform eigen decompositions ($C=F \Lambda F^H$) of matrices ($C$) of a certain *circulant* structure. The `fft` function provides *both* the eigen values of $C$ *and* matrix-vector multiplication $F v$ by an arbitrary vector $v$.
...
u <- rcnorm(length(CB))
dim(u) <- dim(CB)
s <- Re(fft(fft(u)*sqrt(fft(CB)),inverse=TRUE)) / length(CB)
```

According to the previous theory, we could have obtained such a sample alternatively, by noting that `fft(u)` can be replaced by `rcnorm(length(CB), sd=sqrt(length(CB)))`.
