...
We can easily sample from $N(0, C)$.
A sample can then be obtained by scaling a standard gaussian sample with the square root of the covariance matrix:
...
According to the previous theory, we could obtain such a sample alternatively, by noting that `iFFT(u)` can be replaced by complex white noise:

```{r}
uc <- rnorm(length(CB)) + 1i * rnorm(length(CB))
dim(uc) <- dim(CB)
lambda <- fft(CB)
s <- Re( FFT( sqrt(lambda) * uc ) )
```
...
Let's see how if affects the covariance to remove frequencies corresponding to distances greater than 6:
...
The number of frequencies used by this approximation is
...
We can compare the sample `s` with a new sample `s2` based in the low-rank approximation.
Note how we can alternatively express the approximation by setting elements of `uc` to zero:

```{r}
uc[D>Dmax] <- 0
s2 <- Re( FFT( sqrt(lambda) * uc ) )
```

```{r}
image(s2, main="Sample (low-rank)")
```

In conclusion we can use the relatively few `uc[D<=Dmax]` as (complex) 'random effects' to represent the Gaussian Matern field fairly accurately.
