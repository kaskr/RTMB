...
- `MakeADFun` is the *high level* version. This actually creates several internal tapes gathered in a 'model object' often referred to as 'obj'.
- `MakeTape` is the *low level* version. This creates *a single tape*.
...
Derivatives are calculated using the `jacobian()` method. This seeds the derivative of the dependent variable to one, and triggers a *reverse operator loop* effectively updating the entire 'deriv' array:
