...
We want to estimate the two unknown parameters (`muX=5; muN=10`), but the density is not available on closed form. However, we do have access to the moment generating function.

The moment generating function for the Poisson distribution is
...
3. Evaluate the new tape for the remaining (empty) parameter vector. The construct `rep(muX,0)` creates such an empty vector *of the same class as* `muX`.

```{r}
nldens <- function(obs, muX, muN) {
    ## 1
    F <- MakeTape(function(s) {
        K <- Kpois2(s, muX, muN)  ## MGF
